...
<div class="fieldcontain ${hasErrors(bean: taskInstance, field: 'assignTo', 'error')} ">
	<label for="assignTo">
		<g:message code="task.assignTo.label" default="Assign To" />
		
	</label>
	<g:select id="assignTo" name="assignTo.id" from="${tekdays.TekUser.list()}" optionKey="id" value="${taskInstance?.assignTo?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
